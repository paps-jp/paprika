...
Paprika は、被害画像の探索と削除要請という ぱっぷす の活動を支えるために開発し、汎用的な Web 自動化基盤として公開しているものです。こうした技術は目的を問わず使えるため、私たちは公開範囲を意図的に限定しています——被害者画像を検出・特定するロジックは本リポジトリに含めておらず、ページを巡回して画像・動画・リンクを収集する機能のみを公開しています。ご利用にあたっては、以下の方針を必ずお守りください。

**禁止する用途** (要約) — ライセンス (PolyForm Noncommercial) の条件に加え、以下を明確に禁止します。

- 非同意の性的画像 (NCII)・児童性的虐待資料 (CSAM) の収集・保存・拡散
- 特定個人への付きまとい・ストーキング・盗撮・監視を目的とした収集
- サイトのアクセス制御を破る形での利用・不正アクセス (不正アクセス禁止法等)
- 著作権・サイト利用規約に反する収集、および DRM 保護コンテンツの取得

> **bot 判定への対応機能 (`nodriver` 等) について:** これは削除要請のための証拠保全において、対象ページへ人間の閲覧と同じ経路で到達することを目的としたものであり、認証の突破やアクセス制御の回避を意図したものではありません。

Paprika は既定で認証なし (private LAN 内での運用想定) です。外部からアクセス可能な形で運用する場合は、必ずリバースプロキシ + 認証を手前に置いてください。

📖 **禁止用途の詳細、運用者が負うべき責務 (認証・監査ログ・CSAM 検知等)、通報先** は、利用ポリシー全文をお読みください → [マニュアル「利用について」](https://paps-jp.github.io/paprika/#usage)。ご意見・懸念は [ぱっぷす](https://paps.jp) までお寄せください。
