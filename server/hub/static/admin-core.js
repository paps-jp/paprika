...
const I18N_RESOURCES = {
  ja: { translation: {
    ...
    "recovery.salvage.desc":       "自動復旧の永続ログ（MariaDB・全ハブ共通・ハブ再起動でも残る）",
    "recovery.salvage.th.time":    "時刻",
    "recovery.salvage.th.worker":  "ワーカー",
    "recovery.salvage.th.method":  "方法",
    "recovery.salvage.th.result":  "結果",
    "recovery.salvage.th.hub":     "ハブ",
    "recovery.salvage.th.detail":  "詳細",
    "recovery.events.title":       "最近のイベント（ライブ）",
    "recovery.events.desc":        "lifecycle / warn / error を横断表示（このハブが知る範囲・10秒ごと自動更新）",
    "recovery.filter.kind":        "種別:",
    "recovery.filter.range":       "時間範囲:",
    "recovery.filter.worker":      "ワーカー:",
    "recovery.filter.worker.ph":   "worker_id を絞り込み (空 = 全部)",
    "recovery.refresh":            "再取得",
    "recovery.events.th.time":     "時刻",
    "recovery.events.th.kind":     "種別",
    "recovery.events.th.worker":   "ワーカー",
    "recovery.events.th.content":  "内容",
    // Salvage SSH settings card (storage sub-pane)
    "salvage.ssh.title":           "ワーカー自動復旧 SSH (サルベージ用)",
    "salvage.ssh.subtitle":        "ゴーストワーカーを SSH で再起動するための接続情報",
    "salvage.enabled.label":       "自動復旧:",
    "salvage.ssh.user":            "SSH ユーザー:",
    "salvage.ssh.port":            "SSH ポート:",
    "salvage.ssh.keypath":         "秘密鍵パス:",
    "salvage.ssh.keyfile":         "秘密鍵アップロード:",
    "salvage.save":                "設定を保存",
    "salvage.save.ok":             "✓ 保存しました (全ハブへ伝播)",
    "salvage.save.failed":         "保存失敗: ",
    "salvage.key.uploaded":        "✓ 鍵アップロード済み（変更時のみ再アップロード）",
    "salvage.key.none":            "（未アップロード）",
    "salvage.key.readfail":        "鍵読込失敗: ",
    "salvage.key.notpem":          "鍵が PEM 形式ではないようです（PRIVATE KEY が見つからない）",
    "salvage.key.uploadfail":      "鍵アップロード失敗: ",
    "salvage.key.uploadok":        "✓ 鍵をアップロード（全ハブへ伝播）",
    // Hubs sub-tab
    "hubs.heading":         "ハブ",
    "hubs.refresh":         "再読み込み",
    ...
    "ai.subtab.aiio":               "入出力",
    "ai.subtab.aiio.title":         "各 LLM 呼び出しの入出力ログ (planner/codegen/judge/skill 蒸留/作法 蒸留/推論 distiller/perception)",
    "ai.subtab.audit":              "成功監査",
    "ai.subtab.audit.title":        "完了ジョブを VisionAI でサンプリング監査 (本当に正しい動画を取れたか)",
    // Recovery (Workers sub-tab)
    "workers.subtab.workers":       "ワーカー",
    "workers.subtab.hubs":          "ハブ",
    "workers.subtab.recovery":      "リカバリー",
    "workers.subtab.features":      "機能",
    // Host edit modal (excluded / download_video)
    "host.excluded":                "対象外（このサイトは何もしない）",
    "host.excluded.hint":           "「課題(review)」判定も AI エスカレーションも対象から外す。確実に動画が無い／そっとしておきたいサイト向け",
    "host.download_video":          "動画あり（download_video）",
    "host.download_video.hint":     "このサイトは動画サイト。fetch の download_video 未指定(null)時に動画DLを行う。動画の無いサイトは OFF にして重い動画処理(1h timeout)を避ける",
    // Jobs panel — 課題 → exclude button
    "jobs.exclude.btn":             "対象外",
    "jobs.exclude.btn.title":       "このサイト(host)を対象外に登録し、このジョブを課題から外して completed にします（cookie 保持）",
    "jobs.exclude.confirm":         "を「対象外」に登録し、このジョブを課題から外して完了(completed)にします。\n\n今後このサイトは「課題」にも出ず、AI エスカレーションもされません。\n（cookie 等の設定は保持されます）",
    // Skill code modal
    "skills.code.btn":              "コード",
    "skills.code.btn.title":        "このスキルのコード(code_template)を表示",
    "skills.code.copy":             "copy",
    "skills.code.codetemplate":     "code_template",
    "skills.code.codetemplate.hint":"再利用される Python（codegen に注入される本体）",
    "skills.code.llminstructions":  "llm_instructions",
    "skills.code.llminstructions.hint":"プロンプト用の指示文（散文）",
    // AI I/O log sub-pane
    "aiio.heading":                 "AI 入出力ログ",
    "aiio.heading.hint":            "各 LLM 呼び出しの (用途 / engine / prompt / response / latency) — ループ全体の観察用 (10秒ごと更新)",
    "aiio.filter.purpose":          "用途",
    "aiio.filter.engine":           "engine",
    "aiio.filter.job":              "job_id",
    "aiio.filter.since":            "時間範囲",
    "aiio.filter.errors":           "エラーのみ",
    "aiio.refresh":                 "再取得",
    "aiio.th.time":                 "時刻",
    "aiio.th.purpose":              "用途",
    "aiio.th.engine":               "engine",
    "aiio.th.job":                  "job_id",
    "aiio.th.latency":              "latency",
    "aiio.th.io":                   "in/out",
    "aiio.th.preview":              "プロンプト先頭",
    "aiio.modal.prompt":            "プロンプト",
    "aiio.modal.response":          "レスポンス",
    "aiio.modal.error":             "エラー",
    // Success Audit sub-pane
    "audit.heading":                "成功監査",
    "audit.heading.hint":           "完了ジョブを定期サンプリング → VisionAI で「本当に正しい動画?」を判定 (60s 更新)",
    "audit.kpi.audited":            "監査済み",
    "audit.kpi.trueOk":             "✓ 真の成功",
    "audit.kpi.trueOk.title":       "報告も実態も成功",
    "audit.kpi.falsePos":           "✗ 偽陽性",
    "audit.kpi.falsePos.title":     "報告では成功だが実態は失敗 (誤った動画など)",
    "audit.kpi.falseNeg":           "🎯 偽陰性",
    "audit.kpi.falseNeg.title":     "報告では失敗だが実態は成功 (隠れた成功)",
    "audit.kpi.trueFail":           "— 真の失敗",
    "audit.kpi.trueFail.title":     "報告も実態も失敗",
    "audit.kpi.agreeRate":          "報告一致率",
    "audit.kpi.agreeRate.title":    "(真の成功 + 真の失敗) / 全判定 = 報告と実態が一致した率",
    "audit.kpi.trueRate":           "真の成功率",
    "audit.kpi.trueRate.title":     "(真の成功 + 偽陰性) / 全判定 = 実態としての本当の成功率",
    "audit.kpi.topFp":              "偽陽性 top ホスト (download_video 見直し候補)",
    "audit.kpi.topFn":              "偽陰性 top ホスト (judge/escalation 見直し候補)",
    "audit.filter.since":           "時間範囲",
    "audit.filter.verdict":         "判定",
    "audit.filter.disagreeOnly":    "報告不一致のみ",
    "audit.filter.disagreeOnly.title":"偽陽性 OR 偽陰性 (報告と実態の不一致)",
    "audit.run.btn":                "即時実行",
    "audit.run.btn.title":          "次の定期サンプリングを待たずに即時 1 パス実行",
    "audit.th.time":                "時刻",
    "audit.th.reported":            "報告",
    "audit.th.actual":              "実態",
    "audit.th.conf":                "conf",
    "audit.th.job":                 "job_id",
    "audit.th.url":                 "URL",
    "audit.th.video":               "video",
    "audit.th.reason":              "理由",
    "audit.empty":                  "監査結果なし — 「即時実行」で開始するか、Settings で success_audit_enabled を true に",
    // Recovery sub-pane
    "recovery.heading":             "リカバリーログ",
    "recovery.heading.hint":        "worker の lifecycle / warn / error 系イベントを横断表示（このハブが知る範囲・10秒ごと自動更新）",
    "recovery.filter.kind":         "種別",
    "recovery.filter.since":        "時間範囲",
    "recovery.filter.worker":       "ワーカー",
    "recovery.refresh":             "再取得",
    "recovery.th.time":             "時刻",
    "recovery.th.kind":             "種別",
    "recovery.th.worker":           "ワーカー",
    "recovery.th.line":             "内容",
    "recovery.empty":               "該当するイベントはありません",
  ...
  en: { translation: {
    ...
    "workers.subtab.recovery": "Recovery",
    // Recovery sub-pane: salvage history + live events
    "recovery.heading":            "Recovery",
    "recovery.salvage.title":      "Salvage recovery history",
    "recovery.salvage.desc":       "Durable auto-recovery log (MariaDB, shared across hubs, survives hub restart)",
    "recovery.salvage.th.time":    "Time",
    "recovery.salvage.th.worker":  "Worker",
    "recovery.salvage.th.method":  "Method",
    "recovery.salvage.th.result":  "Result",
    "recovery.salvage.th.hub":     "Hub",
    "recovery.salvage.th.detail":  "Detail",
    "recovery.events.title":       "Recent events (live)",
    "recovery.events.desc":        "lifecycle / warn / error across the fleet (what this hub knows; auto-refresh every 10s)",
    "recovery.filter.kind":        "Kind:",
    "recovery.filter.range":       "Time range:",
    "recovery.filter.worker":      "Worker:",
    "recovery.filter.worker.ph":   "Filter by worker_id (empty = all)",
    "recovery.refresh":            "Refresh",
    "recovery.events.th.time":     "Time",
    "recovery.events.th.kind":     "Kind",
    "recovery.events.th.worker":   "Worker",
    "recovery.events.th.content":  "Content",
    // Salvage SSH settings card (storage sub-pane)
    "salvage.ssh.title":           "Worker auto-recovery SSH (salvage)",
    "salvage.ssh.subtitle":        "Connection info to restart ghost workers via SSH",
    "salvage.enabled.label":       "Auto-recovery:",
    "salvage.ssh.user":            "SSH user:",
    "salvage.ssh.port":            "SSH port:",
    "salvage.ssh.keypath":         "Private key path:",
    "salvage.ssh.keyfile":         "Upload private key:",
    "salvage.save":                "Save settings",
    "salvage.save.ok":             "✓ Saved (propagated to all hubs)",
    "salvage.save.failed":         "Save failed: ",
    "salvage.key.uploaded":        "✓ Key uploaded (re-upload only to change)",
    "salvage.key.none":            "(not uploaded)",
    "salvage.key.readfail":        "Key read failed: ",
    "salvage.key.notpem":          "Key does not look like PEM (PRIVATE KEY not found)",
    "salvage.key.uploadfail":      "Key upload failed: ",
    "salvage.key.uploadok":        "✓ Key uploaded (propagated to all hubs)",
    // Hubs sub-tab
    "hubs.heading":         "Hubs",
    ...
    "ai.subtab.aiio":               "I/O Log",
    "ai.subtab.aiio.title":         "Per-LLM-call I/O log (planner/codegen/judge/skill distill/convention distill/reasoning distiller/perception)",
    "ai.subtab.audit":              "Success Audit",
    "ai.subtab.audit.title":        "VisionAI audit of completed jobs (was the right video actually saved?)",
    // Recovery (Workers sub-tab)
    "workers.subtab.workers":       "Workers",
    "workers.subtab.hubs":          "Hubs",
    "workers.subtab.recovery":      "Recovery",
    "workers.subtab.features":      "Features",
    // Host edit modal (excluded / download_video)
    "host.excluded":                "Excluded (do nothing for this site)",
    "host.excluded.hint":           "Skip both 課題(review) classification AND AI escalation. For sites confirmed to have no video, or that you want to leave alone.",
    "host.download_video":          "Has video (download_video)",
    "host.download_video.hint":     "This site is a video site. When a fetch's download_video is unset (null), download video is enabled. Turn OFF on non-video sites to avoid heavy video pipeline (1h timeout).",
    // Jobs panel — 課題 → exclude button
    "jobs.exclude.btn":             "Exclude",
    "jobs.exclude.btn.title":       "Register this host as excluded and remove this job from 課題 by flipping it to completed (cookies preserved).",
    "jobs.exclude.confirm":         " — register as Excluded (do nothing) and flip this job from 課題 to completed?\n\nThis site will no longer appear in 課題 and AI escalation will not fire on it.\n(cookies / settings preserved)",
    // Skill code modal
    "skills.code.btn":              "Code",
    "skills.code.btn.title":        "Show this skill's code (code_template)",
    "skills.code.copy":             "copy",
    "skills.code.codetemplate":     "code_template",
    "skills.code.codetemplate.hint":"Reusable Python body (injected into codegen)",
    "skills.code.llminstructions":  "llm_instructions",
    "skills.code.llminstructions.hint":"Prose advice the LLM sees alongside the code",
    // AI I/O log sub-pane
    "aiio.heading":                 "AI I/O Log",
    "aiio.heading.hint":            "Per-LLM-call (purpose / engine / prompt / response / latency) — end-to-end loop observation (10s auto-refresh)",
    "aiio.filter.purpose":          "Purpose",
    "aiio.filter.engine":           "engine",
    "aiio.filter.job":              "job_id",
    "aiio.filter.since":            "Range",
    "aiio.filter.errors":           "Errors only",
    "aiio.refresh":                 "Refresh",
    "aiio.th.time":                 "Time",
    "aiio.th.purpose":              "Purpose",
    "aiio.th.engine":               "engine",
    "aiio.th.job":                  "job_id",
    "aiio.th.latency":              "latency",
    "aiio.th.io":                   "in/out",
    "aiio.th.preview":              "Prompt head",
    "aiio.modal.prompt":            "Prompt",
    "aiio.modal.response":          "Response",
    "aiio.modal.error":             "Error",
    // Success Audit sub-pane
    "audit.heading":                "Success Audit",
    "audit.heading.hint":           "Periodically sample completed jobs → ask a VisionAI whether the saved video is really the page's main content (60s refresh)",
    "audit.kpi.audited":            "Audited",
    "audit.kpi.trueOk":             "✓ true success",
    "audit.kpi.trueOk.title":       "Report says OK and reality says OK",
    "audit.kpi.falsePos":           "✗ false positive",
    "audit.kpi.falsePos.title":     "Report says OK but reality is NG (wrong video etc.)",
    "audit.kpi.falseNeg":           "🎯 false negative",
    "audit.kpi.falseNeg.title":     "Report says failed but reality is OK (hidden success)",
    "audit.kpi.trueFail":           "— true failure",
    "audit.kpi.trueFail.title":     "Report says failed and reality is failed",
    "audit.kpi.agreeRate":          "Report agreement",
    "audit.kpi.agreeRate.title":    "(true success + true failure) / all decisive = report-vs-reality match rate",
    "audit.kpi.trueRate":           "True success rate",
    "audit.kpi.trueRate.title":     "(true success + false negative) / all decisive = real success rate regardless of report",
    "audit.kpi.topFp":              "False-positive top hosts (download_video review candidates)",
    "audit.kpi.topFn":              "False-negative top hosts (judge/escalation review candidates)",
    "audit.filter.since":           "Range",
    "audit.filter.verdict":         "Verdict",
    "audit.filter.disagreeOnly":    "Disagreement only",
    "audit.filter.disagreeOnly.title":"false positive OR false negative (report disagreed with reality)",
    "audit.run.btn":                "Run now",
    "audit.run.btn.title":          "Run one pass immediately (don't wait for the scheduled interval)",
    "audit.th.time":                "Time",
    "audit.th.reported":            "Reported",
    "audit.th.actual":              "Actual",
    "audit.th.conf":                "conf",
    "audit.th.job":                 "job_id",
    "audit.th.url":                 "URL",
    "audit.th.video":               "video",
    "audit.th.reason":              "Reason",
    "audit.empty":                  "No audit results — click \"Run now\" or set success_audit_enabled = true in Settings",
    // Recovery sub-pane
    "recovery.heading":             "Recovery log",
    "recovery.heading.hint":        "Cross-fleet view of worker lifecycle / warn / error events (scope: what this hub knows about, 10s auto-refresh)",
    "recovery.filter.kind":         "Kind",
    "recovery.filter.since":        "Range",
    "recovery.filter.worker":       "Worker",
    "recovery.refresh":             "Refresh",
    "recovery.th.time":             "Time",
    "recovery.th.kind":             "Kind",
    "recovery.th.worker":           "Worker",
    "recovery.th.line":             "Message",
    "recovery.empty":               "No matching events",
