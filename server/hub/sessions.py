...
class SessionRegistry:
    ...
    async def get_redis_record(self, session_id: str) -> dict | None:
        """Read the full Redis-persisted SessionInfo dict (to_json + hub) for a
        session this hub may not hold locally (P2).

        The worker reconcile (_reconcile_worker_sessions) uses this to rebuild
        and TAKE OWNERSHIP of a job-less session that followed its worker to
        this hub -- after a hub restart the consistent-hash worker-WS route
        sends each worker to a (possibly different) live hub, and that hub must
        re-adopt the worker's sessions rather than orphan-end them. We do NOT
        bulk-reconstruct on startup: that would leave a restarted hub holding
        stale copies of sessions whose workers re-homed elsewhere, fighting the
        new owner over the Redis owner map. Recovery is worker-driven instead --
        the session is rebuilt on whichever hub the worker actually lands on.
        None when redis-less or absent."""
        if self._r is None:
            return None
        try:
            raw = await self._r.get(self._k_session(session_id))
